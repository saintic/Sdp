...
def StartAll(SdpType, **user):
  ...
  if SdpType == "web" or SdpType == "WEB":
    ...
    userinfo_admin = {"name":name, "passwd":passwd, "time":int(time), "service":service, "email":email, 'image':image, 'ip':'127.0.0.1', 'port':int(PORT), 'dn':dn, 'home':userhome}
    conn = dn
  #App start docker
...
<body>
<h1>Welcome %s,以下是您的SdpCloud服务使用信息!</h1>
<p>用户: %s;</p>
<p>密码: %s;</p>
<p>使用期: %d;</p>
<p>服务类型: %s;</p>
<p>验证邮箱: %s;</p>
<p>服务连接信息: %s</p>>
<p>这是一个欢迎页面，请尽快使用SVN或FTP覆盖此页面!</p>
...
''' %(name, name, passwd, int(time), service, email, str(conn))
  ...
  if rc.ping():
    ...
    print '\033[0;32;40m' + '调试输出:', rc.hashget(name), '\033[0m'
    ec.send(*userconn)
    ec.send(*adminconn)
    with open(Public.SDP_UC, 'a+') as f:f.write(json.dumps(userinfo_admin))
    if SdpType == "web" or SdpType == "WEB":
      with open(os.path.join(userhome, 'index.html'), 'w') as f:f.write(userinfo_welcome)
    #import Nginx
  else:
