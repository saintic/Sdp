...
class Docker():
  ...
  def __init__(self):
    ...
    #self.connect = docker.Client(base_url='tcp://127.0.0.1:2375', version='auto')
  ...
  def Create(self, **kw):
    ...
    if volume:
      cfs=['%s:/data:rw' % volume] # ask list ['host_dir:container_dir:mode(rw,ro)']
    else:
      cfs=None
    cports=[]
    cports.append(container_port)
    r=self.connect.create_container(image=image, name=name, stdin_open=True, tty=True, ports=cports, volumes=volume, host_config=self.connect.create_host_config(restart_policy={"MaximumRetryCount": 0, "Name": "always"}, binds=cfs, port_bindings={container_port:host_ip_port}), mem_limit=None, memswap_limit=None, cpu_shares=None)
    cid=r['Id'][:12]
