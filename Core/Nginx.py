...
''' %(Public.SERVER_IP, self.dn, self.ip, int(self.port))
    
    with open(ngx_user_conf, 'w') as f:
      f.write(ngx_conf_content)
    #subprocess.call([Public.NGINX_EXEC + ' -s reload'], shell=True)
    status,output = commands.getstatusoutput(Public.NGINX_EXEC + ' -s reload')
    if status == 0:
      print '\033[0;32;40mSuccess:Reload Nginx                                       \033[0m' + '[' + '\033[0;32;40m确定\033[0m' + ']'
    else:
